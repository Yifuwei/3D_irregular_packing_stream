...
def update_data(data_pool, layout,topos_layout, radio_layout,
                height, T, selected_info, iteration):
...
def ILS_from_the_first_piece(object_info_total, nfv_pool, ifv_pool, max_radio, rho, orientations, ils_orientations, orientations_list,
    ...
    layout, topos_layout, pieces_order = packing_3D_voxel_lookback(original_object_info_total, nfv_pool, ifv_pool, orientations, container_size,
                                                                    container_shape,
    ...
    __x,__y,__z = np.where(topos_layout == 1)
    height = max(__z)
    ...
    trace(f"Constructive algorithm finished, cost {T} s in total, Height is {height}")
    
    no_overlap = np.all(topos_layout <= 1.5)
    ...
    if ALG == "fixed_CA" or ALG == "random_CA" or ALG == "BLF":
        return height, no_overlap, ca_time, len(layout)
    ...
    for each_piece in range(len(object_info_total)):
        each_object_info = layout[each_piece]
        initial_orientations_list_no_bin.append(each_object_info["orientation"])
    
    # data_pool is to track result of each iteration
    data_pool = update_data(data_pool, layout, topos_layout, [],
                height, T, selected_info = "None", iteration="origin")
    ...
    while nonstop:   
        ...
        for each_orientation in other_orientations: 
            ...
            layout, topos_layout, pieces_order = repacking_new_ILS(original_object_info_total, selected_info, nfv_pool, ifv_pool, local_best_orientations_list_no_bin, data_pool, local_best_iteration, ils_orientations, container_size,
                                                                    container_shape, rho, max_radio, 
                                                                    packing_alg, orien_evaluation,
                                                                    SCH_nesting_strategy, density = 5, axis = 'z', 
            ...
            trace(f"=== Re-pack finished! Cost {T} s in this iteration, It takes {meta_TIME} s overall ===")
            # trace(pieces_order)
            
            # no_overlap = all(np.all(each_bin <= 1.5) for each_bin in layout)
            # trace(f"Repack - Overlap: {'No' if no_overlap else 'Yes'}")

            if isinstance(topos_layout,bool):
                perform_this_iter = 99999999

            else:
                __x,__y,__z = np.where(topos_layout == 1)
                perform_this_iter = max(__z)  

            if meta_TIME > time_limit:
                trace("Has reached the time limit, stop!")
                nonstop = False
                break
            
            elif n_iter == iteration_limit and meta_TIME < time_limit:
                ...
                if perform_this_iter < local_best_perform: 
                    ...
                    trace(f"Previous best (height) is {old_best}, now is {local_best_perform}")
                    trace(f"Now best orientations of pieces are: {local_best_orientations_list_no_bin}")

                    data_pool = update_data(data_pool, layout, topos_layout, [],
                            perform_this_iter, T, selected_info, iteration=n_iter)
            ...
            if no_change_time >= kick_trigger_time: 
                ...
                layout, topos_layout, pieces_order = kick_repacking(original_object_info_total, nfv_pool, ifv_pool, orientations, local_best_orientations_list_no_bin, container_size,
                                                                        container_shape, rho, max_radio, 
                                                                        packing_alg, orien_evaluation,
                                                                        SCH_nesting_strategy, density = 5, axis = 'z', 
                                                                        _type = selection_type, _accessible_check = accessible_check, _encourage_dbl = True, 
                ...
                # if isinstance(topos_layout,bool):
                #     perform_this_iter = 99999999
                #     print("kick to a infeasible result")
                ...
                data_pool = update_data(data_pool, layout, topos_layout, [],
                            perform_this_iter, "kick", selected_info = "None", iteration=n_iter)
                ...
                trace(f"Current local best (Height) is {local_best_perform}, kick leads to {perform_this_iter}")
    ...
    no_overlap = np.all(best_topos_layout <= 1.5)
    trace(f"Constructive algorithm - Overlap check: {'Pass' if no_overlap else 'NOT pass!'}")

    if visualisation:
        visualize_voxel_model(best_current_layout, range(0,len(object_info_total)), container_size, container_shape)

    # print(best_current_layout)

    return  global_best_perform, no_overlap, float(ca_time)+float(meta_TIME), len(best_current_layout)
