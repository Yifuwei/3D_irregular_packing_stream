...
from function_lib import insert_add
...
def nesting_evaluation(ongoing, topos_layout, packing_position_list, nesting, bin_size, encourage_dbl):
    # for packing position
    # nesting_evaluation(ongoing_object_info, topos_layout, packing_position_list, nesting, bin_size, _encourage_dbl)

    best_position = (-1,-1,-1)
    best_value = np.inf
    value_list_pairs = []
    topos_copy = copy.deepcopy(topos_layout)
    ...
    for each_position in packing_position_list: 
        ...
        bin_test = insert_add(topos_copy,ongoing["array"],each_position)
        ...
        if nesting == 1: 
            ...
            _x,_y,_z  = np.where(bin_test == 1)
            # value = - max(bin_size[0]*bin_size[1]*(bin_size[2]-z), bin_size[0]*(bin_size[1]-y)*bin_size[2], (bin_size[0]-x)*bin_size[1]*bin_size[2]) + distance_to_0 + (max(_z) ** 3)
            value = max(_z)
...
def SC_heuristic(nfv_pool, ifv_pool, ongoing_object_info, current_layout, topos_layout, max_height_possible, 
    ...
    feasible_region = get_feasible_boundary(topos_layout, current_layout, ongoing_object_info, bin_size, container_shape, nfv_pool, ifv_pool, max_height_possible, flag_NFV_POOL, _accessible_check)
    ...
    best_position = nesting_evaluation(ongoing_object_info, topos_layout, packing_position_list, nesting, bin_size, _encourage_dbl)
