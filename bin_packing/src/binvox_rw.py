...
def read_header(fp):
    ...
    exceptions = []
    try:
        translate = list(map(float, a.strip().split(b' ')[1:]))
    except Exception as e:
        if not e in exceptions:
            exceptions.append(e)
        # print('Warning', e, 'raised.')
    
    a = fp.readline()
    try:
        scale = list(map(float, a.strip().split(b' ')[1:]))[0]
    except Exception as e:
        if not e in exceptions:
            exceptions.append(e)
    if len(exceptions) > 0:
        print('WARNING: Exceptions raised reading this instance:\n', exceptions)
    line = fp.readline()
...
def write(voxel_model, fp):
    ...
    tem = 'dim '+' '.join(map(str, voxel_model.dims))+'\n'
    fp.write(tem.encode())
    
    tem = 'translate '+' '.join(map(str, voxel_model.translate))+'\n'
    fp.write(tem.encode())
    
    tem = 'scale '+ str(voxel_model.scale) + '\n'
    fp.write(tem.encode())
