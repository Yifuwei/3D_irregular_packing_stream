...
def accessibility_check(ongoing_info, nfv, ifv, container_size):
    """_summary_
...
def get_feasible_boundary(fixed, fixed_info, ongoing_info, container_size, container_type, nfv_pool, ifv_pool, flag_NFV_POOL, _accessible_check):
    ...
    # maybe choose the 8 vertices of feasible boundary
        ...
        ifv = ifv_pool.retrieve_ifv(ongoing_info, container_size, container_type)
        ...
        if np.all(intersection < 0.5): # if there is no intersection 
        ...
        if _accessible_check == True: 
            ...
            accessible_region = accessibility_check(ongoing_info, nfv, ifv, container_size)
...
def get_nfv_gpu(fixed, ongoing, dimension, calculations):
    ...
    #     raise RuntimeError(f"CUDA init failed: {e}")
    ...
    NFP = np.zeros(dimension, dtype=np.int8)
    ...
    ongoing_indices = np.column_stack(np.nonzero(ongoing)).astype(np.int8)
